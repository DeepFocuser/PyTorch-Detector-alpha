...
def traindataloader(multiscale=False, factor_scale=[10, 9], augmentation=True, path="Dataset/train",
                    input_size=(512, 512), input_frame_number=2, batch_size=8, batch_interval=10, num_workers=4, shuffle=True,
    ...
    if multiscale:
        ...
        # init = factor_scale[0]
        # end = init + factor_scale[1] + 1
        train_transform = [YoloTrainTransform(y * 32, x * 32, input_frame_number=input_frame_number, mean=mean, std=std, augmentation = augmentation) for y,x in [[10, 20], [17, 30], [24, 40]]]
    else:
...
def validdataloader(path="Dataset/valid",
                    input_size=(512, 512), input_frame_number=2, batch_size=8, num_workers=4, shuffle=True,
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
...
def testdataloader(path="Dataset/test", input_size=(512, 512), input_frame_number=2,
