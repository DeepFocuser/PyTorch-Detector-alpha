...
def run(mean=[0.485, 0.456, 0.406],
    ...
    for anchor, positive, negative, anchor_path, positive_path, negative_path in tqdm(test_dataloader):
        ...
        with torch.no_grad():
            ...
            # cosine similarity
            similarity_of_ap = torch.sum(torch.mul(anchor_pred, positive_pred), dim=1)
            similarity_of_an = torch.sum(torch.mul(anchor_pred, negative_pred), dim=1)
            ...
            ap_rect = cv2.putText(ap_rect, "Simi : " + str(round(similarity_of_ap, 3)), (width // 8, height // 2),
                                  cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1, cv2.LINE_AA)
            an_rect = cv2.putText(an_rect, "Simi : " + str(round(similarity_of_an, 3)), (width // 8, height // 2),
                                  cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1, cv2.LINE_AA)

            ap_catimage = cv2.hconcat([anchor_img, positive_img, ap_rect])
            an_catimage = cv2.hconcat([anchor_img, negative_img, an_rect])

            logging.info(f"similarity_of_ap : {similarity_of_ap}")
            logging.info(f"similarity_of_an : {similarity_of_an}")
